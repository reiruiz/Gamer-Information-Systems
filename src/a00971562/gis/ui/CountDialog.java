...
import a00971562.gis.util.CompareScoreByCount;
...
public class CountDialog extends JDialog {
	 ...
	 */
	public CountDialog() throws SQLException, ClassNotFoundException {
		setTitle("Score List (By Count)");
		...
		if (MainFrame.descending == true) {
			if (MainFrame.gtCondition == true) {
				...
				Collections.sort(scoresList, Collections.reverseOrder(new CompareScoreByCount()));
			} else {
				Collections.sort(scoresList, Collections.reverseOrder(new CompareScoreByCount()));
			}
		} else if (MainFrame.gtCondition == true) {
			UpdateList.filterGamerTag(scoresList, MainFrame.gamertag);
			Collections.sort(scoresList, new CompareScoreByCount());
		} else {
			Collections.sort(scoresList, new CompareScoreByCount());
		}
